...
export function ProductForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const soldCountTrimmed = formData.soldCount.trim();
    const soldCountValue =
      soldCountTrimmed === ""
        ? null
        : (() => {
            const n = parseInt(soldCountTrimmed, 10);
            return Number.isNaN(n) || n < 0 ? undefined : n;
          })();
    if (soldCountTrimmed !== "" && soldCountValue === undefined) {
      toast.error("Jumlah terjual (tampilan) harus angka ≥ 0");
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="soldCount">Manipulasi Sold (Jumlah Terjual Tampilan)</Label>
            <Input
              id="soldCount"
              type="number"
              min="0"
              ...
              placeholder="Kosongkan = pakai jumlah dari order"
            />
